...
{
	this->VERBOSE = false;
	this->originImage.load_bmp(inFileName);
	...
	this->linkedEdge = CImg<unsigned char>(cols, rows, 1, 1, NOEDGE);
	this->result = CImg<unsigned char>(cols, rows, 1, 1, NOEDGE);
...
CImg<unsigned char>& Canny::getOriginImg()
{
	return originImage;
}
...
CImg<short int>& Canny::getDeltaX()
{
	return delta_x;
}

CImg<short int>& Canny::getDeltaY()
{
	return delta_y;
}